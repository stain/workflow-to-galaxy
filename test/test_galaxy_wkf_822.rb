...
class TestGalaxyWkf822 < Test::Unit::TestCase

  def setup
    Dir.chdir("test")
    @wkf = GalaxyTool.new(:wkf_source => Workflows::MYEXPERIMENT_TAVERNA2,
                         :params => {:t2_server => "http://test.mybiobank.org/taverna-server",
                                     :url => 'http://www.myexperiment.org/workflows/822/download/demonstration_of_configurable_iteration_53828.t2flow?version=1' } )
    @wkf.generate
  end
